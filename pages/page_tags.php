...
<head>
  ...
  <link rel="icon" href="icon/<?php echo AppIcon; ?>" sizes="32x32">
	...
	<title><?php echo AppName; ?> : Genre : <?php echo $genre; ?></title>
...
<body>
		...
		<div class="container">
			<div class="row">
				<div class="col-12 col-xl-6">
					<h1 class="section__title section__title--head"><?php echo $genre; ?></h1>
				</div>
            ...
            <li class="breadcrumb__item"><a href="./catalog">Catalog</a></li>
						<li class="breadcrumb__item breadcrumb__item--active"><?php echo $genre; ?></li>
					</ul>
				</div>
			</div>
		</div>
	...
	<div class="catalog catalog--page catalog--list">
		<div class="container">
            ...
            $stmt = $conn->prepare("SELECT * FROM tbl_items WHERE status = 'Visible' AND genres LIKE ? ORDER BY c_id DESC LIMIT $page1,$rpp");
			...
			<div class="row">
              ...
              $stmt = $conn->prepare("SELECT * FROM tbl_items WHERE status = 'Visible' AND genres LIKE ?");
              $stmt->execute([$keywords]);
              $result = $stmt->fetchAll();
              $rec = count($result);
              $total_pages = $rec /$rpp;
