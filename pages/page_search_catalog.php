...
$title = 'Search results for "'.$_GET['keywords'].'"';
}else{
header("location:../");
}
...
<body>
	...
	<section class="section section--head section--head-fixed">
		<div class="container">
            ...
            <li class="breadcrumb__item"><a>Catalog Search</a></li>
						<li class="breadcrumb__item breadcrumb__item--active"><?php echo $_GET['keywords']; ?></li>
					</ul>
				</div>
			</div>
		</div>
	...
	<div class="catalog catalog--page catalog--list">
		<div class="container">
            ...
            $stmt = $conn->prepare("SELECT * FROM tbl_items WHERE status = 'Visible' AND item_id = ? ORDER BY c_id DESC LIMIT $page1,$rpp");
...
$filecountout = 0;
$filesout = glob(dirname(__DIR__)."/admin/core/".$row[3]."out/tree.png");
if ($filesout){
 $filecountout = count($filesout);
              ...
              <div class="col-6 col-sm-4 col-lg-3 col-xl-2">
                <div class="card">
                  <a href="item/<?php echo $row[2]; ?>/<?php echo $item_title; ?>" class="card__cover">
                    <img class="movie_cover" src="img/<?php echo $row[13]; ?>" alt="tree_default">
			...
			<div class="row">
              ...
              $stmt = $conn->prepare("SELECT * FROM tbl_items WHERE status = 'Visible' AND item_id = ?");
              $stmt->execute([$keywords]);
              $result = $stmt->fetchAll();
              $rec = count($result);
              $total_pages = $rec /$rpp;
