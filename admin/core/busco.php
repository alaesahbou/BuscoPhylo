...
$output = shell_exec('source ~/.bashrc ; for file in '.$path.'/'.$data["posts"][0]['input'].'*.'.$data["posts"][0]['type'].' ; do output=$(basename ${file%%.'.$data["posts"][0]['type'].'*}) ; sudo busco -i $file -l '.$data["posts"][0]['lineage'].' -o $output -m '.$data["posts"][0]['mode'].' --out_path  '.$path.'/'.$data["posts"][0]['input'].'outBusco >> '.$path.'/'.$data["posts"][0]['input'].'busco.log ; done ');
...
if(isset($data["posts"][0]['outgroup']) && ($data["posts"][0]['outgroup']=="" || is_numeric($data["posts"][0]['outgroup']))){
     $output = shell_exec('cd '.$path.'/'.$data["posts"][0]['input'].'; sudo python3 '.$path.'/scripts/script_phylo.py -d busco -o out -t 20 > '.$path.'/'.$data["posts"][0]['input'].'phylo.log');
   ...
   $output = shell_exec('cd '.$path.'/'.$data["posts"][0]['input'].'; sudo python3 '.$path.'/scripts/script_phylo.py -og '.$data["posts"][0]['outgroup'].' -d busco -o out -t 20 >> '.$path.'/'.$data["posts"][0]['input'].'phylo.log');
   shell_exec('sudo touch '.$path.'/'.$data["posts"][0]['input'].'out/s.sh');
        $myfile = fopen($_SERVER["DOCUMENT_ROOT"]."/admin/core/".$data["posts"][0]['input']."out/s.sh", "w") or die("Unable to open file!");
        $txt = "#!/bin/bash\n";
        fwrite($myfile, $txt);
...
$output = shell_exec('cd '.$path.'/'.$data["posts"][0]['input'].' ; sudo zip -r Result.zip *');
